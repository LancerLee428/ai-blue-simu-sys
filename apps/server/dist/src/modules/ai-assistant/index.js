...
import { confirmScenarioDeployment, recordScenarioDraftRegenerated, rejectScenarioDraft, } from '../scenario-workspace';
export const aiAssistantModule = {
    key: 'ai-assistant',
    title: 'AI Assistant',
    description: '上下文问答、结构化辅助与智能部署入口。',
};
function createDraftId(command) {
...
export function createDeploymentDraft(command) {
    const scenarioName = command.scenarioId === demoScenarioWorkspaceState.scenario.id
...
export function createDeploymentDraftResponse(command) {
    return {
        command,
        draft: createDeploymentDraft(command),
    };
}
export function confirmDeploymentDraft(command) {
    const workspace = confirmScenarioDeployment(command.items.map((item) => ({
...
export function rejectDeploymentDraft(command) {
    return {
