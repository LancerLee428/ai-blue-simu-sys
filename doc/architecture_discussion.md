...
### 8.0 当前系统定义与架构讨论范围重新确认

> [!IMPORTANT]
> 在进入 P0 开放问题的逐项决策前，需要先重新收敛一个更基础的问题：**我们到底在讨论什么系统，以及当前这份文档讨论的是这个系统的哪一层问题。**
> 如果这个前提不先钉死，后续关于 MVP、想定、工作台、推演、AI 的讨论很容易彼此替代，导致范围漂移。

#### 8.0.1 系统定义：我们要建设的是“智能蓝军平台”，不是单点工具

当前文档所讨论的系统，不应被理解为以下任一单点系统：
...
> **一个以 Ontology 对象模型为中枢、以威胁建模与演训蓝军为首期核心场景、并可逐步扩展到规则推演、专业仿真引擎和 AI 辅助分析的智能蓝军平台。**

这个定义有三层含义：

1. **它是平台，不是功能点集合。**  
   平台的核心特征是统一数据底座、统一对象模型、统一工作流上下文和统一能力挂载点，而不是若干页面或若干接口的拼装。

2. **它的架构核心在平台协同层，而不在专用推演内核本身。**  
   平台负责对象建模、关系组织、任务上下文、态势展示、AI 协同与权限审计；高保真裁决与专业求解能力由规则引擎、仿真引擎和优化引擎逐步承接。

3. **它是可分阶段演进的系统，而不是第一阶段一次性做完的系统。**  
   第一阶段的交付只是平台能力骨架的首批落点，不代表平台的全部边界。

#### 8.0.2 首期核心场景：以威胁建模为底座，以演训蓝军为交付抓手

当前文档讨论的首期目标，不应被误读为“先做一个想定产品”或“先做一个地图部署工具”。

更准确的理解应当是：

- **威胁建模**负责定义平台底座，即统一对象模型、关系组织方式和数据语义边界
- **演训蓝军**负责提供首期最可见、最可操作、最能体现平台价值的业务抓手

因此，第一阶段的重点不是孤立做某一个模块，而是验证下面这个命题：

> **以 Ontology 为中枢的平台架构，是否能够支撑威胁建模与演训蓝军场景的首期落地。**

在这个前提下，想定管理、态势工作台、对象查询、地图部署都应被视为首期交付抓手，而不是系统定义本身。

#### 8.0.3 当前文档的讨论范围：平台级技术架构与分阶段实现路线

当前这份文档讨论的重点，应明确限定为以下三个层次：

1. **系统定位与平台边界**  
   我们到底要做一个什么平台，它与仿真系统、情报系统、指挥系统、AI 系统之间的边界在哪里。

2. **平台级技术架构**  
   包括分层结构、Ontology 核心地位、平台与专业引擎边界、AI 的受控位置、数据与展示的耦合方式等。

3. **分阶段实现路线**  
   第一阶段做什么来验证平台骨架；第二阶段如何引入规则与轻量推演；第三阶段如何引入 AI 助手与战例知识；后续如何扩展到边缘部署与多域协同。
...
这些内容可以在后续专题文档中展开，但不应替代当前文档的主线。

#### 8.0.4 对后续讨论的约束

基于以上重新确认，后续所有技术架构讨论应默认遵循以下约束：

1. **先讨论平台定义，再讨论首期抓手；不要用首期交付物反向定义整个系统。**
2. **先讨论能力骨架和分层边界，再讨论模块细节与实现手段。**
3. **首期交付必须服务于平台能力验证，而不是退化为一个局部工具。**
4. **想定、态势工作台、对象建模、AI 辅助都属于平台的一部分，但没有任何单一模块可以代表整个平台。**

基于这一前提，下面的 P0 开放问题分析，均应被理解为：

> **围绕“智能蓝军平台”的首期实现路线，而不是围绕某个局部产品的功能优先级。**
...
> [!IMPORTANT]
> 在明确“当前文档讨论的是智能蓝军平台，而非某个局部工具”的前提下，第一阶段不应再被理解为“做完整平台”或“做单点模块”，而应被定义为：**建设智能蓝军平台的首个可运行骨架。**
>
> 这一阶段的目标不是把所有能力做完，而是把平台最不可绕过、最可见、最能承接后续演进的骨架立起来。

#### 8.4.1 第一阶段建设目标

第一阶段需要验证的，不是某个页面能否跑通，也不是某个局部功能是否完整，而是下面这个更核心的命题：

> **以 Ontology 为中枢的平台架构，是否能够支撑威胁建模与演训蓝军场景的首期落地，并通过受控 AI 助手体现智能化交互与智能化部署能力。**

因此，第一阶段应被理解为：

- 平台骨架的首个可运行形态
- 平台首期核心场景的可见交付
- AI 助手能力从第一版开始真实进入前后端骨架
- 后续规则引擎、仿真引擎挂载前的基础底座

#### 8.4.2 第一阶段必须立住的五根骨架

##### 1. 作战语义骨架（Ontology Seed）

这是整个系统的脊柱。第一阶段必须建立种子级对象模型，而不是退化成若干页面各自维护自己的对象定义。
...
##### 2. 场景构建骨架（Scenario Workspace）

这是第一阶段最重要的业务抓手。平台必须有一个可操作的场景工作空间，用来承接演训蓝军的首期业务落地。
...
这里的重点不是做一个孤立的“想定模块”，而是形成一个真正承载对象上下文、部署结果和编辑状态的工作空间。

##### 3. 态势可视化骨架（Situation Workbench）

这是第一阶段最重要的可见交付面。平台不仅需要管理对象和场景，还必须能在统一工作台中承载空间态势展示。
...
第一阶段的态势工作台应收敛为“场景态势工作台”，而不是一开始就做成完整指挥沙盘或推演沙盘。

##### 4. AI 助手骨架（AI Assistant Layer）

AI 不应再被理解为第一阶段的弱预留能力，而应成为首版骨架中的真实组成部分。其价值不只是提供解释和摘要，更要体现系统“智能蓝军平台”的智能化特征。

第一阶段 AI 层至少应支持以下三类能力：

- **上下文感知问答**：能够基于当前想定、当前对象、当前区域、当前态势和相关知识片段回答问题，而不是做脱离业务上下文的通用聊天
- **结构化智能辅助**：能够生成想定摘要、对象摘要、风险提示、部署建议草案、场景检查提示等结构化结果
- **智能化部署入口**：能够通过 Chat 形式接收用户意图，例如“在台湾部署对抗兵力”，并将其转化为受控的部署草案或部署动作链路，辅助完成想定部署

这里尤其需要强调“智能化部署”的定位：

1. AI 可以理解自然语言中的部署意图，例如区域、对象类型、部署目标、对抗方向等
2. AI 可以结合 Ontology、兵力装备源、GeoZone、当前想定上下文生成部署方案草案
3. AI 可以调用受控工具链完成对象筛选、区域定位、部署建议和投影生成
4. AI 输出的部署结果必须经过规则校验、权限校验、必要的人机确认与审计记录，不能绕过主业务链路直接成为系统真相

因此，第一阶段的 AI 不应只是一个外挂聊天框，而应是：

- 前端有明确入口的智能助手
- 后端有明确边界的 AI 应用服务层
- 能读取对象、场景、区域、文档上下文的受控智能协同层
- 能支持“问答 + 解释 + 草案 + 智能部署”的综合能力

##### 5. 平台治理骨架（Access / Audit / Version）

这是最容易被忽略，但第一阶段必须内建的基础能力。智能蓝军平台天然要求：
...
没有这层，后续 AI、规则、推演、复盘都缺少可信基础。

#### 8.4.3 第一阶段在九层架构中的覆盖范围

##### 需要真正立起来的层

- **C. 作战语义层**：必须立，作为核心底座
- **E. AI 助手层**：必须立，作为首版就要体现“智能化交互与智能化部署”的核心能力层
- **F. 任务与工作流层**：部分立，优先支持想定管理、场景工作上下文、基础版本流转
- **G. 展示层**：必须立，优先支持地图态势、对象详情、基础关系展示、AI 助手入口
- **H. 安全治理层**：基础立，优先支持 RBAC、审计日志、基础权限域、AI 输出审计

##### 需要轻量立起来的层

- **A. 数据接入层**：轻量立，优先支持结构化导入、JSON/CSV、手工维护
- **B. 数据治理层**：轻量立，优先支持基础清洗、来源标识、基础标签

##### 只做接口预留、不做主体实现的层

- **D. 推演与分析引擎层**：规则引擎、仿真引擎、优化求解器仅做接口预留
- **I. 运维部署层**：只保留基础部署能力，不进入复杂平台工程建设

#### 8.4.4 第一阶段能力范围：必须有 / 应该有 / 明确不做
...
- 多域协同
- 边缘节点与复杂部署
- AI 主动决策链路（绕过权限、规则与人工确认的自治执行）

#### 8.4.5 第一阶段的收敛表达

第一阶段的系统能力可以收敛为一句话：

> **第一阶段要做的是：围绕种子级 Ontology，建立面向演训蓝军场景的想定工作空间、态势工作台与受控 AI 助手能力，并补齐最小治理能力，形成“可操作、可解释、可智能部署、可扩展”的智能蓝军平台首个可运行骨架。**

这意味着：

- 平台定义上，以 Ontology 为底座
- 产品交付上，以场景工作空间为抓手
- 可见呈现上，以态势工作台为载体
- 智能体现上，以受控 AI 助手承接问答、解释、草案与 Chat 式智能部署
- 架构策略上，以规则、仿真、AI 的后续深化挂载为演进方向

在此基础上，后续所有工程结构、模块划分与实现优先级，都应围绕这四根骨架展开，而不应回退为“先做一个局部工具”的思路。
